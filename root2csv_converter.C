...
void root2csv_converter() {
	...
	TString txt_out="train,tag,target,ntrklen,trklen,Etrklen,PID,B,costheta,mediandedx,endpointdedx,calo,avcalo,keffbeam,keffhy,kendbb,kendfitbb,st_x,st_y,st_z,end_x,end_y,end_z,pbdt,nd";
	...
	//for(int i=0; i<20; i++){
	  ...
	  fout<<train<<","<<tag<<","<<TARGET<<","<<ntrklen<<","<<trklen<<","<<Etrklen<<","<<PID<<","<<B<<","<<costheta<<","<<mediandedx<<","<<endpointdedx<<","<<calo<<","<<avcalo<<","<<keffbeam<<","<<keffhy<<","<<kend_bb<<","<<kend_fit_bb<<","<<st_x<<","<<st_y<<","<<st_z<<","<<end_x<<","<<end_y<<","<<end_z<<","<<pbdt<<","<<nd<<"\n";	
	}
  	fout.close();
	//fout_train.close();
	//fout_test.close();
