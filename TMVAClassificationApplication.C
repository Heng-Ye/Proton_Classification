...
void TMVAClassificationApplication(TString myMethodList, TString fname, TString foutname) {
	cout << endl;
	...
	reader->AddVariable("ntrklen", &ntrklen); 
	reader->AddVariable("trklen", &trklen); 
	reader->AddVariable("PID", &PID); 
	reader->AddVariable("B", &B); 
	reader->AddVariable("mediandedx", &mediandedx); 
	reader->AddVariable("endpointdedx", &endpointdedx); 
	reader->AddVariable("calo", &calo); 
	reader->AddVariable("avcalo", &avcalo); 
	...
	//Noth from HY: As long as one run the ROC analysis for the method(s), the text files of weights will be automatically created
	for (std::map<std::string,int>::iterator it = Use.begin(); it != Use.end(); it++) {
		if (it->second) {
			TString methodName = TString(it->first) + TString(" method");
			TString weightfile = dir + prefix + TString("_") + TString(it->first) + TString(".weights.xml");
			cout<<"Loading reader:"<<methodName<<endl;
	...
	if (!gSystem->AccessPathName( fname )) {
		...
		cout<<"ooPS.. nO input file found!"<<endl;	
	...
	theTree->SetBranchAddress( "B", &B);
	theTree->SetBranchAddress( "mediandedx", &mediandedx);
	theTree->SetBranchAddress( "endpointdedx", &endpointdedx);
	theTree->SetBranchAddress( "calo", &calo);
	theTree->SetBranchAddress( "avcalo", &avcalo);
	...
	for (Long64_t ievt=0; ievt<theTree->GetEntries();ievt++) {
		...
		bool IsRecoInEL_BDT=false;
		if (ntrklen<min_norm_trklen_csda) { //inel region
			if (PID>pid_1) IsRecoInEL=true;
			//if (pid<=pid_1) IsRecoEL=true;
		} //inel region
		if (ntrklen>=min_norm_trklen_csda&&ntrklen<max_norm_trklen_csda) { //stopping p region
	...
	TFile *target  = new TFile(Form("%s",foutname.Data()),"RECREATE" );
	...
	std::cout << "--- Created root file:"<<foutname<<" containing the MVA output histograms" << std::endl;
